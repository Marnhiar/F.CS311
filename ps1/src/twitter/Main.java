...
//        	tweets = PostReader.readJsonFile("D:\\OneDrive - Mongolian University of Science and Technology\\Hicheel\\"
//        			+ "Programuud\\Python\\GroupPosts.txt");
        ...
        final Map<String, Integer> keywordCount = SocialNetwork.keywordCount(tweets, keyword);
        Object[] person = keywordCount.keySet().toArray();
        System.out.println("\nKeyword - " + keyword );
        int total = 0;
        for (int i=0; i<person.length; i++) {
        	int temp = keywordCount.get(person[i]);
        	System.out.println(person[i] + " = " + temp);
        	total += temp;
        }
        System.out.println("Total included times = " + total);
    }
